...
{
	std::string line;
	std::ifstream file("board_1.txt");
	char chars[16];

	if (file.is_open()) // if the file is open, if not, open it!
	{
		while (std::getline(file, line)) // copies line by line in the file and gives it to the string line
		{
